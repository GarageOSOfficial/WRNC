...
describe('MOTOR VIN lookup client', () => {
  ...
  it('validates VIN before session or network access', async () => {
    ...
    await expect(lookupMotorVin('bad-vin', fetcher)).rejects.toMatchObject({ code: 'INVALID_VIN' });
  ...
  it('normalizes proxy errors without leaking response details', async () => {
    ...
    await expect(lookupMotorVin('1HGCM82633A004352', fetcher)).rejects.toEqual(expect.objectContaining<Partial<MotorLookupError>>({ code: 'MOTOR_CONTRACT_REQUIRED', status: 503 }));
  });
});
