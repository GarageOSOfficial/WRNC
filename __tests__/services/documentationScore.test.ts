...
describe('documentation score service', () => {
  ...
  it('calculates category scores from existing records', () => {
    ...
    expect(activityCategory?.evidence).toEqual(['Activity history present', 'Activities have detail']);
    expect(photoCategory?.evidence).toEqual(['No evidence found for this category']);
  });
