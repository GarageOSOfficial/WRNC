...
export default function VehicleDocumentsRoute() {
  ...
  const { data: documents = [], isLoading: documentsLoading } = useDocuments(vehicle?.workspaceId, { includeArchived: true });
  ...
  if (vehicleLoading || !vehicle) {
    return <SafeAreaView className="flex-1 items-center justify-center bg-wrnc-background px-6"><Text className="text-sm text-wrnc-text-secondary">Loading documents…</Text></SafeAreaView>;
  }

  return (
    <SafeAreaView className="flex-1 bg-wrnc-background">
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 16, paddingBottom: 40 }} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
        ...
        <View className="mt-4 rounded-2xl border border-wrnc-border bg-wrnc-surface p-5">
          ...
          <Text className="mt-2 text-sm text-wrnc-text-secondary">{vehicle.nickname || `${vehicle.year} ${vehicle.make} ${vehicle.model}`}</Text>
          <View className="mt-4">
            <DocumentUploadForm
              ...
              onSubmit={async ({ title, category, file }) => {
                ...
                await uploadDocument.mutateAsync({ workspaceId: vehicle.workspaceId, vehicleId, userId, title, category, file });
              }}
            />
              ...
              documents.map((document) => (
                <DocumentCard
                  ...
                  onPress={async () => {
                    setOpenError(null);
                    try {
                      const url = document.storagePath ? await getDocumentSignedUrl(document.storagePath) : document.fileUrl;
